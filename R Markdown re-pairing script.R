...
absol_hist$female_ring.x<-as.factor(absol_hist$female_ring)
...
#M1: female boldness_male boldness- assortative mating
#create transformed response variable and scale the female boldness scores

absol_hist$male_sqrt<-sqrt(max(absol_hist$male_pers.x +1 )-absol_hist$male_pers.x) # correct for negative skew
absol_hist$male_sqrt2<- max(absol_hist$male_sqrt+1)-absol_hist$male_sqrt # return figures to correct direction

absol_hist_g<-ggplot(absol_hist, aes (x = male_sqrt2))+ # check success
  geom_histogram()

absol_hist$female_pers_sc<-scale(absol_hist$female_pers.x) # scale female pers variable

#check for colinearity
colin_model <- glm(male_sqrt2~  colony +female_pers_sc, data = absol_hist)
summary(colin_model)
...
mating_model<-lmer(male_sqrt2 ~ female_pers_sc+
                     colony+
                     colony:female_pers_sc+
                     (1|female_ring.x), #female id random effect
...
## @knitr mating_graph_data

# reconfiguring the data produced by the model for visualisation
mating_model_g<-plot_model(mating_model, type = "pred", terms=c ("female_pers_sc[all]", "colony")) # extract the data from the model

mating_model_g<-as.data.frame(mating_model_g[["data"]]) # convert to a data frame
mating_model_g$female_pers_sc<-mating_model_g$x # rename variables
mating_model_g$male_pers_sc<-mating_model_g$predicted
mating_model_g$colony<-mating_model_g$group

## @knitr mating_graph

#plot graph
mating_plot2<-ggplot()+
  geom_point(data=absol_hist, aes(x = female_pers_sc, y = male_sqrt2, fill = colony), colour ="black",pch=21, size = 4)+
  geom_ribbon(data=mating_model_g, aes(x = female_pers_sc, ymin = conf.low, ymax = conf.high, colour = colony,fill = colony), alpha= 0.2)+
  geom_line(data=mating_model_g, aes(x = female_pers_sc, y = male_pers_sc, colour = colony, linetype = colony), linewidth = 1.5)+
  scale_linetype_manual(values = c("solid","dashed"))+
  scale_colour_manual(values=c("black", "black"))+
  scale_fill_manual(values=c("black", "lightgrey"))+
  theme_classic()+
  xlab("Scaled female boldness score")+
  ylab("Male boldness score (square root transformed)")+
...
#An additional model featuring female boldness is retained alongside the model featuring only absolute difference and the null model.
#This model has an AICc equal to that of the null model and so was ultimately exclude, however it continues to appear
#in the results produced by this code.  

repro_pers_coeffs<-coefTable(nest_repro_pers, full=TRUE) # create final coefficient tables
repro_pers_coeffs

## @knitr model_2_sim_mod
...
repro_pers_plot1<-ggplot()+ geom_point(data= repro_pers, aes(x = absol, y = chick_sur_num), alpha = 0.2, size = 4)+
 ...
 geom_line(data=repro_pers_plot1, aes(x = absol, y = chick_sur), size = 1.5)+
 theme_classic()+
...
repro_plotg2<-ggplot()+ 
  ...
  geom_errorbar(data= repro_plot2, aes(x = chick_sur2, ymin = conf.low, ymax = conf.high), width = 0.3, size = 0.8)+
...
# grumant
set.seed(100)# to ensure randomisation results are repeatable
...
# If p < 0.05, 
# we can infer that focal birds are choosing partners who are closer to their
# personality than we would expect by chance 
...
abline(v = mean_trueDiff, col = "red", lty = "dotted") # represents observed mean difference or new partners
abline(v = 0, col = "purple") # represents original mean difference or old partners 

mean(as.numeric(perms_output))
abline(v = -0.28, col = "black") # observed mean difference which would be expected if choosing partners at random
...
mean(as.numeric(perms_output))
abline(v = 0.12, col = "black") # observed mean difference which would be expected if choosing partners at random
